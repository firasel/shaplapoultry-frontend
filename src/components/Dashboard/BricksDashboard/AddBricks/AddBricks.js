...
const AddBricks = () => {
  ...
  const onSubmit = (data) => {
    ...
    setIsLoading(true);
    fetch("https://shaplapoultrysapi.herokuapp.com/bricks/add", {
      method: "POST",
      headers: {
      ...
      .then((data) => {
        if (data.status) {
          SuccessNotify("Bricks Add Successful");
        } else {
          ErrorNotify("Bricks Not Added,Please Try Again");
        }
        setIsLoading(false);
      })
      .catch((error) => {
